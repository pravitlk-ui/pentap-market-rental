...
// ---------- Design tokens ----------
// bg: #F2F4F1 (sage paper) | surface: #FFFFFF | ink: #1F2A24 (forest ink)
// brand: #2F6F4E (awning green) | accent: #E8A33D (turmeric) | alert: #C1502E (terracotta)
// muted border: #D8DED9
...
// dome groups: A/B/C = 1.5x3 m, D = 3x3 m
const DOME_GROUPS = {
  A: { size: "1.5×3 ม." },
  B: { size: "1.5×3 ม." },
  C: { size: "1.5×3 ม." },
...
// generic next-number helper for a single flat prefix (shophouse / outdoor)
...
// dome codes are grouped by leading letter (A/B/C/D), each with its own numbering
...
// figure out the color/label a unit should show — paid daily and paid monthly
// use different colors; unpaid (same-day/same-month) shows as overdue as a reminder.
// Actual reset to "vacant" for stale daily units happens once per day at load (see rollover below).
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("pentap-market-units", false);
        let list;
        let isFresh = false;
        if (res && res.value) {
          list = JSON.parse(res.value).map(u => ({ rentType: "monthly", payments: [], occupiedDate: null, ...u }));
        } else {
          list = seedUnits();
          isFresh = true;
        }
        // daily rollover: a daily unit left "occupied" from a previous day (never
        // touched today) opens back up to vacant so it can be rented again today
        ...
        setUnits(list);
        if (isFresh || changed) {
          await window.storage.set("pentap-market-units", JSON.stringify(list), false);
        }
      } catch (e) {
        const seeded = seedUnits();
        setUnits(seeded);
        try { await window.storage.set("pentap-market-units", JSON.stringify(seeded), false); } catch (_) {}
      } finally {
        setLoading(false);
      }
    })();
  ...
  const persist = useCallback(async (next) => {
    setUnits(next);
    try {
      const result = await window.storage.set("pentap-market-units", JSON.stringify(next), false);
      if (!result) showToast("บันทึกไม่สำเร็จ ลองใหม่อีกครั้ง");
    } catch (e) {
  ...
  // keep `selected` in sync with the underlying units array (so payment log updates reflect live)
...
function Overview({ overview }) {
  ...
  return (
    <div>
      ...
      {/* actual money collected — click a card to see the underlying payment list */}
...
function PentaPLogo({ size = 40 }) {
  // five petals, colors ordered clockwise from top to roughly match the Penta P farm mark
